...
const Post = (props) => {
  ...
  const {
    ...
    caption,
    _id: id,
    postedBy: { _id: postedId, username, dp },
    likes,
    comments,
  } = props.post;
  ...
  const handleShare = (e) => {
    const id = e.target.parentElement.parentElement.parentElement.id;
    const username =
      e.target.parentElement.parentElement.parentElement.dataset.username;
    let link = String(window.location).split("/", 3);
    alert(
      `Share with your friends -> ${link[0]}//${link[2]}/${username}/${id}`
    );
  };
    ...
    >
      <div className="post-top-part">
        <Link to={`/user/${username}`}>
          <img
            ...
            width="40"
            height="40"
            src={dp || require("../images/profile-pic.jpg")}
            alt="profile pic"
          />
        </Link>
        ...
        {username === mydata.username ? (
          <i
            onClick={(e) => M.Dropdown.init(e.target).open()}
            data-target="dropdown1"
            style={{ padding: "0.5rem 1rem" }}
            className="post-top-more fas fa-ellipsis-h"
          ></i>
        ) : (
          <i
            onClick={(e) => M.Dropdown.init(e.target).open()}
        ...
        {username === mydata.username ? (
          <ul id="dropdown1" className="dropdown-content">
            <li className="dropdown-options" onClick={handleDelete}>
              Delete
            </li>
            <li className="dropdown-options" onClick={handleShare}>
              Share
            </li>
          </ul>
        ) : (
      ...
      <div className="post-top-part">
        {
          ...
          likes.find((my) => my.username === mydata.username) === undefined ? (
            ...
            <i
              onClick={like}
              className="fa red-text fa-heart fa-2x"
              aria-hidden="true"
            ></i>
          )
        }
        ...
        {mydata.favPosts.find((eachFav) => eachFav === id) === undefined ? (
          <i onClick={handleFavPost} className="far fa-bookmark fa-2x"></i>
        ) : (
        ...
        <h6 className="post-liked-username">
          {
            ...
            likes.length === 0 ? null : likes.length === 1 ? (
              <>
                Liked by{" "}
              ...
              <>
                ...
                <Link to={`/user/${likes[1].username}`}>
                  <strong>{likes[1].username}</strong>
                </Link>
              </>
            ) : (
              <>
                ...
                and <Link to={`/user/${likes[1].username}`}></Link>{" "}
                <Link to={`/${id}/likes`}>
      ...
      </div>
      {
        ...
        comments.slice(-1, comments.length).map((each) => {
          ...
          return (
            <div
              key={each.by._id + Math.random()}
              className="post-top-part post-comments"
            >
              <h6
                className="post-comment-username"
                style={{ paddingBottom: "0.2rem" }}
              >
                ...
                <Link to={`/user/${each.by.username}`}>
                  <strong>{each.by.username}</strong>
                </Link>
              </h6>
      ...
      <div className="post-top-part post-comments">
        <img
          ...
          width="18"
          height="18"
          src={mydata.dp || require("../images/profile-pic.jpg")}
          alt="profile pic"
        />
